...
local function config(plugin, opts)
  ...
  if ok_3f then
    toggleterm.setup({})
    local function _2_()
      local group = _G.vim.api.nvim_create_augroup("TermAU", {})
      _G.vim.api.nvim_create_autocmd({"FileType"}, {group = group, pattern = {"toggleterm"}, command = "setlocal nospell"})
      _G.vim.api.nvim_create_autocmd({"TermOpen"}, {group = group, command = "setlocal nospell"})
      _G.vim.keymap.set("t", "<esc>", "<C-\\><C-n>", {noremap = true, silent = true, nowait = true, buffer = 0})
      _G.vim.keymap.set("t", "<C-h>", "<Cmd>wincmd h<CR>", {noremap = true, silent = true, nowait = true, buffer = 0})
      _G.vim.keymap.set("t", "<C-j>", "<Cmd>wincmd j<CR>", {noremap = true, silent = true, nowait = true, buffer = 0})
      _G.vim.keymap.set("t", "<C-k>", "<Cmd>wincmd k<CR>", {noremap = true, silent = true, nowait = true, buffer = 0})
      _G.vim.keymap.set("t", "<C-l>", "<Cmd>wincmd l<CR>", {noremap = true, silent = true, nowait = true, buffer = 0})
      return _G.vim.keymap.set("t", "<C-w>", "<C-\\><C-n><C-w>", {noremap = true, silent = true, nowait = true, buffer = 0})
    end
    return _G.vim.api.nvim_create_autocmd({"TermOpen"}, {pattern = "term://*", callback = _2_})
  else
    return nil
  end
end
